...
import * as service from "../services/contactMessage";
...
export async function list(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const isRead = req.valid?.query?.isRead as boolean | undefined;
    const { items, total } = await service.list({ ...pg, paintingId, isRead });
